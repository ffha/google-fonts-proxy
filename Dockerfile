...
RUN apk add gcc openssl-dev wget zlib-dev make gd-dev geoip-dev pcre2-dev git libc-dev
WORKDIR /usr/src
...
RUN make -j $(nproc)
RUN make install
RUN apk del zlib-dev geoip-dev pcre2-dev openssl-dev gd-dev gcc wget libc-dev git
RUN rm -rf /usr/src/*
